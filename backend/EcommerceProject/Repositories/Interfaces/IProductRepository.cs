...
namespace EcommerceProject.Repositories.Interfaces
{
    ...
    {
        Task<ProductCatalogResponse> GetPagedAsync(int? categoryId, string? search,string?categoryName, List<string>? tags, decimal? minPrice, decimal? maxPrice, int page, int pageSize, string sortOrder, bool onlyActive, CancellationToken ct);
        ...
        Task<List<int>> GetRelatedProductIdsAsync(int categoryId, int excludeProductId, int take, CancellationToken ct);
